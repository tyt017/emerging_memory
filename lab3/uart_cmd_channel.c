...
extern UINT8 protectEnable_flag;
...
void process_uart_cmd_channel(UINT8 bFlag)
{
	...
	while (xyz[0] == '\r' || xyz[0] == '\n')
 	...
 	else if (strncmp(xyz, "write", 5) == 0)
 	{
    	...
    	// Set Buffer
    	memset((byte *)(FLASH_MEM_BASE), bPATTERN, 4096);
        pSRamBuffer[0] = 0;
 	...
 	else if (strncmp(xyz, "read", 4) == 0)
 	{
      ...
      for (j = 0; j <= 255; j++)
      {
      	...
      	// pSRamBuffer[0] = (byte) wPageNo;
      	UARTprintf(" UART pBuf[%d] 0x %08X %08X %08X %08X\n", i, pSRamBuffer[i+0], pSRamBuffer[i+1], pSRamBuffer[i+2], pSRamBuffer[i+3]);
 	...
 	{
 		UARTprintf(" HELLO LAB 1\n");
 		UARTprintf(" Ya-Tong TSAI\n");
 		UARTprintf(" P76131181\n");
 		UARTprintf(" 2024/10/17\n");
 	}
 	else if (strncmp(xyz, "protectEnable", 13) == 0)
 	{
 		UARTprintf(" Write Protect Enable\n");
 		protectEnable_flag = 1;
 	}
 	else if (strncmp(xyz, "protectDisable", 14) == 0)
 	{
 		UARTprintf("Write Protect Disable\n");
 		protectEnable_flag = 0;
 	}
 	else
	{
		if(bFlag == 0)
